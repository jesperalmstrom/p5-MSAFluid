      ...
      for(int iy=1;iy<rows-1;iy++) {
        ...
        if(a>= minRegisterFlowVelocity) 
        {
          ...
          if(a>=minDrawParticlesFlowVelocity) { 
            ...
            float kinectNormX = (x0+u) * invKWidth;// / kWidth;
            float kinectNormY = (y0+v) * invKHeight; // kHeight;
            float kinectVelX = ((x0+u) - x0) * invKWidth;// / kWidth;
            float kinectVelY = ((y0+v) - y0) * invKHeight;// / kHeight;         
